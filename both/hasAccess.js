 ...
 * has access to a plan's features. Will return if they
 * either have the plan or they have a plan that includes
 * the plan.
 *
 * @method  AppPlans.hasAccess
 * @public
 * @reactive
 ...
 * @returns {Boolean} Has access to plan's features?
 */
AppPlans.hasAccess = Meteor.wrapAsync(function (planName, options, callback) {
  ...
  function doesHaveAccess(planList) {
    return _.any(planList, function (plan) {
      var includedList = Utility.getPlan(plan.planName).includedPlans || [];
      return plan.planName === planName || _.contains(includedList, planName);
    });
  }

  var list = Utility.getPlansList(options, function (error, list) {
    callback(error, doesHaveAccess(list));
  });

  return list && doesHaveAccess(list);
});
